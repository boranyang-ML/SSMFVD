...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    Arguments:
        class_logits : predicted prob info，shape=[num_anchors, num_classes]
        box_regression : predicted bbox regression
        labels : true label
        regression_targets : true bbox
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            if gt_boxes_in_image.numel() == 0:  # background
                # background image
                ...
                # calculate the iou between proposal and each gt_box
