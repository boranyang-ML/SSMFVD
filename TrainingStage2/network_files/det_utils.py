...
class BoxCoder(object):
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        ctr_x = boxes[:, 0] + 0.5 * widths   # anchor/proposal x coordinates
        ctr_y = boxes[:, 1] + 0.5 * heights  # anchor/proposal y coordinates

        wx, wy, ww, wh = self.weights  # RPN: [1,1,1,1], fastrcnn: [10,10,5,5]
        dx = rel_codes[:, 0::4] / wx   # predict anchors/proposals x coordinates regression
        dy = rel_codes[:, 1::4] / wy   # predict anchors/proposals y coordinates regression
        dw = rel_codes[:, 2::4] / ww   # predict anchors/proposals width regression
        dh = rel_codes[:, 3::4] / wh   # predict anchors/proposals height regression
...
class Matcher(object):
    ...
    def __call__(self, match_quality_matrix):
        """
        calculate the max iou between anchors and gtboxes, and record the index
