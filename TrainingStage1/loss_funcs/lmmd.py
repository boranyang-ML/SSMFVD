...
class LMMDLoss(MMDLoss, LambdaSheduler):
    ...
    def forward(self, source, target, source_label, target_label): #
        if self.kernel_type == 'linear':
            ...
            batch_size = source.size()[0]
            weight_ss, weight_tt, weight_st = self.cal_weight(source_label, target_label) #
    ...
    def cal_weight(self, source_label, target_label):
        #print("###############################call-weight function print###############################")
        batch_size = source_label.size()[0]
        source_label = source_label.cpu().data.numpy()
        # [0 1 0 1 0 0 1 1 0 ... 1] len() = batch_size
        #print("source_label", source_label)
        source_label_onehot = np.eye(self.num_class)[source_label]  # one hot
        # [1 0 0] [0 1 0] [1 0 0] ... 基于source_label 转为对应的one hot向量
        #print("source_label_onehot_1", source_label_onehot)

        source_label_sum = np.sum(source_label_onehot, axis=0).reshape(1, self.num_class)
        source_label_sum[source_label_sum == 0] = 100
        source_label_onehot = source_label_onehot / source_label_sum  # label ratio
        # 先计算在这64个（取决于batch size）样本中，每一个class分别有多少个样本，
        # 然后source_label_onehot里面的每一个“1”就变成了“1/对应class的样本个数”
        #print("source_label_onehot_2", source_label_onehot)

        # real target domain label
        target_label = target_label.cpu().data.numpy()
        #print("target_label", target_label)
        target_label_onehot = np.eye(self.num_class)[target_label]  # one hot
        #print("target_label_onehot_1", target_label_onehot)
        ...
        #print("target_label_onehot_2", target_label_onehot)
        ...
        for i in range(self.num_class):  # (B, C)
            if i in set_s and i in set_t:
                s_tvec = source_label_onehot[:, i].reshape(batch_size, -1)  # (B, 1)
                t_tvec = target_label_onehot[:, i].reshape(batch_size, -1)  # (B, 1)
